---
title: "CAAEV5V6ExtMmrCCDataExtensionParmPublisher"
type: "interface"
module: "CAAV5V6MechanicalModeler"
category: "framework"
base: "CATParmPublisherAdapter"
inheritance_chain: "CAAEV5V6ExtMmrCCDataExtensionParmPublisher → CATParmPublisherAdapter"
method_count: 0
visibility: "local"
has_tie_binding: false
verified: true
---
# CAAEV5V6ExtMmrCCDataExtensionParmPublisher

**基类**: CATParmPublisherAdapter  
**继承链**: CAAEV5V6ExtMmrCCDataExtensionParmPublisher → CATParmPublisherAdapter  
**模块**: CAAV5V6MechanicalModeler  
**分类**: framework  
**可见性**: local  
**方法数**: 0

> KnowledgeInterfaces Framework

## 说明

该接口作为标记接口或配置接口使用，无自定义方法。

## 依赖

- `CATIAV5Level.h`
- `CATParmPublisherAdapter.h`

