...
tags: ["CAAScrBase", "CAAPriChangeHole06", "CAAPriChangeHole04", "CAAPriChangeHoleFormSource", "CAAScdInfUseCases", "CAAPriChangeHoleModuleSource", "CAAPriChangeHole03", "CAAScdPriUseCases", "CAAPriChangeHole01", "CAAScrJavaScript", "CATIAHole", "CAAPriChangeHoleModule", "CAAlink", "CAAPriChangeHole07", "CAAPriChangeHole02", "CAAInfLauchMacro", "CAAPriChangeHole", "CAAPriChangeHoleForm", "CATIA"]
source_file: "Doc/online/CAAScdPriUseCases/CAAPriChangeHoleVBA.htm"
converted: "2026-05-11T11:06:32.831060"
---
...
---
 ...
 |[]This macro is dedicated for Windows only, and it is
 the catvba version of [Changing the Hole
 Parameters].

 The macro is made of two files [CAAPriChangeHoleModule.bas]
 which contains the starting procedure CATMain and [CAAPriChangeHoleForm.frm]
 ...
 file. Data unit is take into account: Millimeter or Inches.

 This macro allows you to define:
 

 
- T**he limit type: "Up to
 next" or "Offset" onl**y.
 
- **Maximum and minimum**
 tolerances of the hole diameter.
 
- "Simple" or "Counterbored" as hole type only, m**aximum
 and minimum** tolerances for the spot facing diameter for
 counterbored holes.
 
- Whether the hole is threaded or not, and its **thread
 depth** if threaded.
 
- A parameter named "Hole_Description" where its value is a
 letter corresponding to the applied parameters.
 
 

It modifies the [*Hole*]
 object from its methods and properties, and updates the part.
...
- []Open the [CAAPriChangeHole.CATPart]
 document.
 
- Reference in the application the catvba project: [CAAPriChangeHole.catvba].
 
- The text file [CAAPriChangeHole.txt]
 is located in the CAAScdPriUseCases module.
...
Load the CAAPriChangeHole.CATPart that contains three holes.
 

![](images/CAAPriChangeHole01.gif)
 

Select one or several holes as shown. Selection is allowed from the
 specification tree or from the geometry.
 

![](images/CAAPriChangeHole02.gif)
...
If 
oPartDocument.Selection.Count = 0
 Then
...
' ------------
 ...
 MsgBox "Select the holes you wish to transform before running the macro.", 
vbOKOnly
, "Warning"
...
```
...
Set 
oCATIAFileSys = CATIA.FileSystem
...
Set 
oFile = oCATIAFileSys.GetFile(sDocPath & "\online\CAAScdPriUseCases\macros\CAAPriChangeHole.txt")
...
Set 
oTextSteam = oFile.OpenAsTextStream("ForReading")
...
...
 ...
 Set 
oParameters = oPartDocument.Part.Parameters.SubList(oHole, True)
...
' ------------
 ...
 If 
ParameterExists("Hole_Description", oParameters) = True Then
...
Case
 mfgDescription.TextMatrix(iRow, 6)
 
Case
 "Yes"

 If 
oHole.Diameter.Value < oHole.ThreadDiameter.Value 
And _
 
 oHole.BottomLimit.Dimension.Value > oHole.ThreadDepth.Value
 Then
