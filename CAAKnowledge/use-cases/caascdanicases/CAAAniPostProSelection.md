...
![](../CAAScrBase/images/atarget.gif) |  This use case shows how groups are selected image visualization. The macro opens an Analysis document which already contains a computed frequency case and images. Selections are set for the Von Mises Stress image which already exists in the document. Different groups such as mesh parts, clamps and groups are selected.   ![](images/ImageSelection.gif)    
---|---  
![](../CAAScrBase/images/ainfo.gif) |  CAAAniPostProSelection is launched in CATIA [1]. No open document is needed. [ CAAAniPostProSelection.catvbs](CAAAniPostProSelectionSource.htm) is located in the CAAScdAniUseCases module. [Execute macro](macros/CAAAniPostProSelection.catvbs) (Windows only).    
![](../CAAScrBase/images/ascenari.gif) |  CAAAniPostProSelection includes the following steps:
...
According to the general [ Analysis Document](../CAAScdAniTechArticles/CAAAniTocAnalysisDocument.htm) structure, this macro uses some standard procedures to navigate or retrieve the required objects. First, from the **Document** , we find the **Analysis Manager Object** , and the **Analysis Model**. 
