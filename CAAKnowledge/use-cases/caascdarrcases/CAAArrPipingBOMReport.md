...
![](../CAAScrBase/images/atarget.gif) | This example shows how to extract a report of Arrangement objects within a document. Help is taken of a macro. This macro has a reference to CATIAArrBOMReport, CATIAV5ArrWorkbench Interfaces (referred to as ArrBOMReport, Workbench in the reference Visual Basic document).This macro opens an existing Product document and retrieves the CATIAArrWorkbench selection. Then it finds the interface CATIAArrBOMReport. It then generates the BOM report and saves the report.  
---|---  
![](../CAAScrBase/images/ainfo.gif) | CATArrPipingBOMReport is launched after CATIA is up and the reference document is opened.CATArrPipingBOMReport.CATScript is located in the runtime directory Operating System (say intel_a)\code\command  
![](../CAAScrBase/images/ascenari.gif) | CAAArrPipingBOMReport includes the following steps:
