...
tags: ["CAAScrBase", "CATIASchAppConnectable", "CAASCH_RouteBetween2Equip", "CAASchRouteBetween2Equip", "CATIASchCompatible", "CATIASchCntrLocation", "CAASCHEDU_SamplePID", "CATIASchGRR", "CATIASchCompGraphic", "CATIASchListOfBSTRs", "CATIASchAppConnector", "CAASCHEDUFuncString", "CATIASchRouteGraphic", "CAAScdSchUseCases", "CATIAProduct", "CATIASchGRRComp", "CATIA", "CAASCHEDUConnector"]
source_file: "Doc/online/CAAScdSchUseCases/CAASchRouteBetween2EquipSource.htm"
converted: "2026-05-11T11:06:32.630434"
---
...
SchGRRComp
 ...
 Set 
objSchTempListFact = objSchRoot.GetTemporaryListFactory

 If 
( Not ( objSchTempListFact Is Nothing ))
 Then

 Set 
objLCompat_g = objSchTempListFact.CreateListOfObjects

 Set 
objLGRRComp_g = objSchTempListFact.CreateListOfObjects

 End If

 If 
( Not ( objLCompat_g Is Nothing ) And _
 Not ( objLGRRComp_g Is Nothing ) )
 Then
...
SchListOfObjects
 ...
 If 
( Not ( objSchRouteGraphArg Is Nothing ) )
 Then
...
( Not ( objSchLGRR Is Nothing ) )
 ...
 Set 
GetRoutePrimitives = objSchLGRR.Item (1,"CATIASchGRR")
...
Private Function FindConnectorAtPosition ( dbXArg
 As 
Double, dbYArg
 As 
Double, _
...
SchGRR
 ...
 If 
( Not ( objSchCntblArg Is Nothing ) And _
...
objLFilter = Nothing
 ...
 Set 
objLCntr = objSchCntblArg.AppListConnectors (objLFilter)

 Set 
objSchRouteGraphic = objSchRootArg.Ge
tInt
erface ( _
 "CATIASchRouteGraphic", objSchCntblArg)

 If 
( Not ( objSchRouteGraphic Is Nothing ) )
 Then

 Set 
objGRR = GetRoutePrimitives (objSchRouteGraphic)

 End If

 End If 
'--- If ( Not ( objSchRoute Is Nothing ) ...

 If 
( Not ( objLCntr Is Nothing ) And _
 Not ( objGRR Is Nothing ) )
 Then
...
objLDbOut = Nothing
 ...
 Set 
objCntrLoc = objLCntr.Item (iCntr,"CATIASchCntrLocation")

 If 
(Not ( objCntrLoc Is Nothing ) )
 Then

 objCntrLoc.GetPosition objGRR,objLDbOut

 If 
( Not ( objLDbOut Is Nothing ) )
 Then

 
 Int
NbCoord = objLDbOut.Count
...
( ( dbXOut = dbXArg ) And _ 
 ...
 Set 
FindConnectorAtPosition = objSchRootArg.Ge
tInt
erface ( _
...
Private Sub Find2ComponentInst (objSchRootArg
 As 
SchematicRoot)

 If 
( objLCompat_g Is Nothing ) Then Exit Sub

 If 
( objLGRRComp_g Is Nothing ) Then Exit Sub
...
( Not ( objSchRootArg Is Nothing ) )
 ...
 Set 
objLCompInst = objSchRootArg.GetComponents

 If 
( Not ( objLCompInst Is Nothing ) )
 Then

 
 int
NbComp = objLCompInst.Count
...
Integer
 ...
 Set 
objCompCompatFrom = Nothing

 Set 
objGRRCompFrom = Nothing
...
Found = 0
 ...
 Set 
objCompCompat = objLCompInst.Item (intIndex,"CATIASchCompatible")

 If 
( Not ( objCompCompat Is Nothing ) )
 Then

 Set 
objPrd = objSchRootArg.Ge
tInt
erface ( _
...
( Not ( objPrd Is Nothing ) )
 ...
 int
Found = Instr (1, strInstName, strTgtFrom, 1) 
...
Found < 1 )
 ...
 int
Found = Instr (1, strInstName, strTgtTo, 1) 
...
SchCompGraphic
 ...
 Set 
objSchCompGraph = objSchRootArg.Ge
tInt
erface ( _
 "CATIASchCompGraphic",objCompCompat)

 Set 
objGRRComp = GetComponentImage (objSchCompGraph)

 If 
( ( Not objGRRComp Is Nothing ) )
 Then
...
StoreIndex = 1 )
 ...
 Set 
objCompCompatFrom = objCompCompat

 Set 
objGRRCompFrom = objGRRComp
 Else
...
(
 int
NbFound > 1 ) Then Exit For 

 End If 
'--- If ( Not ( objCompCompat Is Nothing ) ...

 Next

 If 
( Not ( objCompCompatFrom Is Nothing ) And _
 Not ( objGRRCompFrom Is Nothing ) )
 Then
...
Is Nothing ) And _
 ...
 End If 
'--- If (Not ( objLCompInst Is Nothing ) ...
...
Private Sub RouteLineBetween2Component (objSchRootArg
 As 
SchematicRoot)
...
Integer
 ...
 int
NbComp = objLCompat_g.Count
 
 int
NbGRR = objLGRRComp_g.Count

 If 
(
 int
NbComp <> 2 ) Then Exit Sub
...
NbGRR ) Then Exit Sub
 ...
 If 
( objSchRootArg Is Nothing ) Then Exit Sub
...
SchAppObjectFactory
 ...
 Set 
objAppObjFact = objSchRootArg.GetApplObjFactFromVirtualType ("CAASCHEDU_SamplePID")

 If 
( objAppObjFact Is Nothing ) Then Exit Sub
...
SchBaseFactory
 ...
 Set 
objSchBaseFact = objSchRootArg.GetSchBaseFactory 

 If 
( objSchBaseFact Is Nothing ) Then Exit Sub
...
objPrd = Nothing 
 ...
 Set 
objCompCompat = objLCompat_g.Item (intIndex,"CATIASchCompatible")

 Set 
objGRRComp = objLGRRComp_g.Item (intIndex,"CATIASchGRRComp")

 Set 
objPrd = objSchRootArg.Ge
tInt
erface ("CATIAProduct",objCompCompat)
...
Index = 1 )
 ...
 If 
( Not ( objGRRComp Is Nothing ) And _
 Not ( objCompCompat Is Nothing ) )
 Then
...
'---------------------------------------------------------------------
 ...
 Set 
objSchGRR = objSchRootArg.Ge
tInt
erface ("CATIASchGRR",objGRRComp) 

 If 
( Not ( objLCntrs Is Nothing ) And _
 Not ( objSchGRR Is Nothing ) And bCompatible )
 Then
...
Index = 1 )
 ...
 Set 
objAppCntrCompBest1 = objAppCntrBest
 strMessage_g = strMessage_g & _
 ...
 Set 
objAppCntrCompBest2 = objAppCntrBest
...
( Not ( objAppRouteRef Is Nothing ) )
 ...
 If 
( Not ( objSchRoute Is Nothing ) )
 Then

 strMessage_g = strMessage_g & "schematic route created" & vbCr

 Set 
objRouteCntbl = objSchRootArg.Ge
tInt
erface ( _
...
'-----------------------------------------------------------------------
 ...
 Set 
objAppCntrRouteBest1 = FindConnectorAtPosition ( _
 db2CntrPt1(0), db2CntrPt1(1), objRouteCntbl, objSchRootArg)

 Set 
objAppCntrRouteBest2 = FindConnectorAtPosition ( _
...
'-----------------------------------------------------------------------
 ...
 If 
( Not (objAppCntrRouteBest1 Is Nothing ) And _
 Not (objAppCntrCompBest1 Is Nothing ) )
 Then
...
'--------------------------------------------------------------------
 ...
 Set 
objAppConnection = objAppCntrCompBest1.AppConnect _
 (objAppCntrRouteBest1)

 If 
( Not ( objAppConnection Is Nothing ) )
 Then
 ...
 End If 
'--- If ( Not (objAppCntrRouteBest Is Nothing ) ...

 If 
( Not (objAppCntrRouteBest2 Is Nothing ) And _
 Not (objAppCntrCompBest2 Is Nothing ) )
 Then
...
'--------------------------------------------------------------------
 ...
 Set 
objAppConnection = objAppCntrCompBest2.AppConnect _
 (objAppCntrRouteBest2)
...
'--- If ( Not ( objSchRoute Is Nothing )...
 ...
 End If 
'--- If ( Not ( objAppCompRef Is Nothing ) ...
