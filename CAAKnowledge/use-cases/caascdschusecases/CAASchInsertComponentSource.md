...
tags: ["CAAScrBase", "CATIASchAppConnectable", "CATIASchCompatible", "CAASCH_Sample", "CAASCH_RouteForPlacement", "CAASacInsertComponent", "CATIASchComponent", "CATIASchCompGraphic", "CATIASchGRRRoute", "CAASchInsertComponent", "CATIASchRouteGraphic", "CATIASchRoute", "CAAScdSchUseCases", "CATIA"]
source_file: "Doc/online/CAAScdSchUseCases/CAASchInsertComponentSource.htm"
converted: "2026-05-11T11:06:32.578885"
---
...
'---------------------------------------------------------------------
 ...
 Set 
objSchCntblCVRef = objSchGRRCVCtlg.GetSchObjOwner

 If 
( Not ( IsEmpty (objSchCntblCVRef ) ) )
 Then
...
Boolean 
 ...
 Set 
objSchCompCVRef = objSchRoot.GetInterface ("CATIASchComponent",objSchCntblCVRef)

 If 
( Not ( IsEmpty (objSchCompCVRef ) ) )
 Then

 strMessage = strMessage & "Got catalog component reference of the symbol" & vbCr

 Set 
objSchCompatRoute = FindARouteInModel (objSchRoot)

 End If 
'If ( Not ( IsEmpty (objSchCompCVRef ) ) ...

 If 
( Not ( IsEmpty (objSchCompCVRef ) ) And _
 Not ( IsEmpty (objSchCompatRoute )) )
 Then
...
' -- step 1 
 ...
 Set 
objCompRefPlaceInfo = objSchCompCVRef.QueryConnectAbility _
 (objSchGRRCVCtlg) 
...
' -- step 3 
 ...
 If 
( Not ( IsEmpty (objFinalInsertInfo ) ) )
 Then
...
' -- step 4 
 ...
 If 
( Not ( IsEmpty (objSchCompInst ) ) And _
 Not ( IsEmpty (objSchRouteInst ) ) )
 Then
...
( Not ( IsEmpty (objSchRouteInst ) ) )
 ...
 Set 
objSchCntblRouteInst = objSchRoot.GetInterface ( _
 "CATIASchAppConnectable",objSchRouteInst)

 Set 
objSchRouteGraph = objSchRoot.GetInterface ( _
 "CATIASchRouteGraphic",objSchRouteInst)

 Set 
objLDbPlace = FindPlacementPoint (objSchRoot, objSchRouteGraph)
 

 End If

 If 
( Not ( IsEmpty (objSchCntblRouteInst ) ) And _
 Not ( IsEmpty (objLDbPlace ) ) )
 Then
...
CATSafeArrayVariant
 ...
 If 
( Not ( IsEmpty (objSchCntblRouteInst ) ) )
 Then
 ...
 End If 
'---- If ( ( Not ( IsEmpty (objSchCntblRouteInst ) ) ...

 End If 
'----If ( Not ( IsEmpty (objSchCompCVRef ) )...

 End If 
'---- If ( Not ( IsEmpty (objSchCntblCVRef ) )...

 End If 
'----- If ( Not ( IsEmpty (objSchGRRCVCtlg ) )...

 End If 
'----If ( Not ( IsEmpty (objSchRoot ) )...
...
Private Function FindARouteInModel (objSchRootArg
 As 
SchematicRoot)
 As 
SchCompatible
...
SchListOfObjects
 ...
 If 
( Not ( IsEmpty (objSchRootArg ) ) )
 Then

 Set 
objSchLSymbols = objSchRootArg.GetRoutes

 If 
( Not ( IsEmpty (objSchLSymbols ) ) )
 Then

 Set 
FindARouteInModel = objSchLSymbols.Item (1,"CATIASchCompatible")
...
Private Function FindPlacementPoint (objSchRootArg
 As 
SchematicRoot, _ 
 objSchRouteGraphArg
 As 
...
( Not ( IsEmpty (objSchRootArg ) ) )
 ...
 Set 
objSchTempListFact = objSchRootArg.GetTemporaryListFactory

 If 
( Not ( IsEmpty (objSchTempListFact ) ) )
 Then

 Set 
FindPlacementPoint = objSchTempListFact.CreateListOfDoubles
 ...
 If 
( Not ( IsEmpty (objSchRouteGraphArg ) ) And _
 Not ( IsEmpty (FindPlacementPoint ) ) )
 Then
...
objSchLGRR = objSchRouteGraphArg.ListGraphicalPrimitives
 ...
 If 
( Not ( IsEmpty (objSchLGRR ) ) )
 Then

 Set 
objSchGRRRoute = objSchLGRR.Item (1,"CATIASchGRRRoute")

 If 
( Not ( IsEmpty (objSchGRRRoute ) ) )
 Then

 objSchGRRRoute.GetPath objSchLDb

 If 
( Not ( IsEmpty (objSchLDb ) ) )
 Then
...
( intCount > 3 )
 ...
 End If 
'--- If ( Not ( IsEmpty (objSchLGRR ) ) ...

 End If 
'--- If ( Not ( IsEmpty (objSchRouteGraphArg ) ) ...
