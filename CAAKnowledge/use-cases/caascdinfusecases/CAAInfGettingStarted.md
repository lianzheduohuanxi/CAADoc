...
tags: ["CAAScrBase", "CAAInfRecord2", "CAAScdInfTechArticles", "CAAInfRecord4", "CAAInfEdit1", "CAAInfEdit2", "CAAInfRecord7", "CAAScdInfUseCases", "CAAInfAddingMacroInToolbar", "CAAInfRecord6", "CAAInfGettingStartedSource", "CAAInfInvoking", "CAAInfRecord3", "CATIASketch3", "CAAInfReplay2", "CAAInfGettingStarted", "CAAInfRecord5", "CATIA"]
source_file: "Doc/online/CAAScdInfUseCases/CAAInfGettingStarted.htm"
converted: "2026-05-11T11:06:32.779625"
---
...
![](../CAAScrBase/images/awarning.gif)
...
This scenario creates a circle in a sketch, and uses this sketch to
 create a cylindric pad.
...
Select the **Tools->Macro->Start
 Recording ... ** command to display the Record Macro dialog 
 box:
...
> 
 ...
 ![](images/CAAInfRecord2.jpg)
 

A default macro library is provided. Macro libraries are places where 
 a macro can be stored, they can be folders, VBA project or Documents.
 
 

Depending in the library, you can select different recording 
...
Click **Start** in the **Record Macro** dialog box
 to start recording the macro. The **Stop Recording** dialog box 
 appears.
...
- In the **File **menu, click on **New,** or click on
 the
 ![](images/I_NewP2.gif) icon,
...
- Select the xy plane in the specification tree, and select
 the sketcher icon
 ![](images/I_SketcherP2.gif) to
 create a sketch.
 
 
 |![](images/CAAInfRecord4.jpg)
...
- In the Pad Definition dialog box, choose a length of 20 mm
 and click OK. The pad is created.
 
 
 |![](images/CAAInfRecord6.jpg)
 
 
 

 
- The pad creation is now complete.
 
 
 |![](images/CAAInfRecord7.jpg)
 
 
 

 
- Click **Stop Recording** in the **Stop Recording** 
 dialog box, or select the **Tools->Macro->Stop Recording** command. Your macro is now stored in the file
 you have selected.
...
Set
 circle2D1 = factory2D1.CreateClosedCircle(0.000000, 0.000000, 10.000000)
...
## []3-Modifying the Generated Macro
 ...
 |![](images/atarget.gif)
 |This task explains how to modify the generated
 macro to make it loop on the creation of five identical cylindric pads.
...
- Select the **Tools->Macro->Macros... command **to display the **Macros** dialog box.
...
- Select the name of the macro you have just created and 
 click on **Edit** to display the **Macro Editor** window.
...
![](../CAAScrBase/images/tips.gif)
 |You can choose your own text editor to edit the macro by  setting
 the **CATMacroEditor** environment variable prior to launching CATIA
 with the name of the editor program:
```
...
On Unix. This editor must be accessible through the **PATH**
...
- The instructions written using the bold typeface are
...
You simply need to initialize a variable, here x, to allow for the
 sketch position in the plane to vary, and create a loop beginning with
 the `For` keyword and ending with the **`Next`**
 keyword. The `For` keyword specifies the counter variable `I`
 ...
 not change. Change the first parameter of the `CreateCloseCircle`
 method to `x`. Increment the value of the `x`
 variable to move the next center of 25mm from the previous one. 
...
- Save the macro using the **File->Save** command of the 
 Macro Editor and exit the editor using the **File->Exit** command.
 
 
 
 
 |The source of the modified macro, [CAAInfGettingStarted.CATScript],
 is available in the CAAScdInfUseCases module. [Execute
 macro] (windows only).
...
This task explains how to run the modified macro.
 ...
 click **Run** to run this macro. Here is the result.

![](images/CAAInfReplay2.jpg)
