---
title: "ApplicationFrame Modifications"
category: "use-case"
module: "CAACenAPIChangesR10"
tags: "["CATICSOFilter"]"
source_file: "Doc/online/CAACenAPIChangesR10/ApplicationFrame.htm"
converted: "2026-05-11T17:33:50.084848"
---
tags: ["CATICSOFilter"]
source_file: "Doc/online/CAACenAPIChangesR10/ApplicationFrame.htmmd"
converted: "2026-05-11T17:33:50.084848"
CAA C++ API Modifications|  ApplicationFrame  |

* * *

**Entity|  SP| Modification| To Do** | ApplicationFrame/Protected/CATICSOFilter.h/CATICSOFilter/AvailableElements| GA| NPVM| Implement it when entity is implemented or inherited
---|---|---|---
