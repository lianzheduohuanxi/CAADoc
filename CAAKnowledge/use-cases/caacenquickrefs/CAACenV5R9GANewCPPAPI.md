---
```vbscript
title: "New C++ Authorized APIs in CAA V5R9 GA"
category: "use-case"
module: "CAACenQuickRefs"
tags: ["CATIFunctActionsGroup", "CATITPSView", "CATIAVPMVDAImport", "CATIFunctVariant", "CATIMfgTPModification", "CATISchRouteAlternateGraphic", "CATIFunctAction", "CATIKnowledgeAdvisorAddin", "CATIMfgGeometryTabPage", "CATIVpmSCLogin", "CATIValue", "CATISchGRRRouteAlternate", "CATIFunctVariants", "CATISmgMachiningAreaChecks", "CATIFunctDocument", "CATIMfgManufacturingUtilities", "CATIFunctElement", "CATIInstance", "CATISchAppReplace", "CATIFunctFacetMgr"]
source_file: "Doc/online/CAACenQuickRefs/CAACenV5R9GANewCPPAPI.htm"
converted: "2026-05-11T17:33:47.676659"
```

---
tags: ["CATIFunctActionsGroup", "CATITPSView", "CATIAVPMVDAImport", "CATIFunctVariant", "CATIMfgTPModification", "CATISchRouteAlternateGraphic", "CATIFunctAction", "CATIKnowledgeAdvisorAddin", "CATIMfgGeometryTabPage", "CATIVpmSCLogin", "CATIValue", "CATISchGRRRouteAlternate", "CATIFunctVariants", "CATISmgMachiningAreaChecks", "CATIFunctDocument", "CATIMfgManufacturingUtilities", "CATIFunctElement", "CATIInstance", "CATISchAppReplace", "CATIFunctFacetMgr"]
source_file: "Doc/online/CAACenQuickRefs/CAACenV5R9GANewCPPAPI.htm"
converted: "2026-05-11T17:33:47.676659"
CAA V5 Encyclopedia |  New C++ Authorized APIs in CAA V5R9 GA

* * *

The following are the new CAA V5R9 GA C++ Authorized APIs, compared with CAA V5R8 at GA level.

  * AdvancedMathematics framework
    * CATMathFoundationX
    * CATMathFoundationXY
    * CATMathFunctionGeneral
  * AnalysisMeshingModel framework
    * CATIMSHCriterion
    * CATMSHConnecInfos
    * CATMSHConnectivityServices
    * CATMSHCriterionInfos
    * CATMSHElementVisuAttribute
    * CATMSHExtIConnectivity
    * CATMSHExtICriterion
    * CATMSHGeometricalEngine
    * CATMSHNodeVisuAttribute
  * AnalysisMeshingToolsUI framework
    * CATIMSHAdvWorkbenchAddin
  * ApplicationFrame framework
    * AddAccessChild
    * CATIProgressTask
    * CATIProgressTaskUI
    * CATTaskController
    * E_BADARGUMENT
    * SetAddinMenu
  * CATAnalysisBase framework
    * CATAnalysisCollectorRoot
    * CATAnalysisEntityCollector
  * CATAnalysisInterfaces framework
    * CATISamSensor
    * CATSamSelectionMode
  * CATFunctSystemItf framework
    * CATFunctOrientationDirection
    * CATIFunctAction
    * CATIFunctActions
    * CATIFunctActionsGroup
    * CATIFunctActionsGroups
    * CATIFunctAssociation
    * CATIFunctAssociations
    * CATIFunctDescription
    * CATIFunctDocument
    * CATIFunctElement
    * CATIFunctFacet
    * CATIFunctFacetManagers
    * CATIFunctFacetMgr
    * CATIFunctGenScriptMgr
    * CATIFunctMultiRepMgr
    * CATIFunctNodeGraphLayout
    * CATIFunctObject
    * CATIFunctObjectProxy
    * CATIFunctObjects
    * CATIFunctPosition
    * CATIFunctScript
    * CATIFunctScripts
    * CATIFunctVariant
    * CATIFunctVariants
  * CATIAApplicationFrame framework
    * CATExtISelectShow
    * CATICafTreeSettingAtt
    * GetCATCafTreeSettingCtrl
  * CATSchPlatformInterfaces framework
    * CATEASchApp2DZoneFrom3DZone
    * CATEASchAppAssembly
    * CATEASchAppAssemblyMember
    * CATEASchAppCntrShow
    * CATEASchAppDisplayMode
    * CATEASchAppReplace
    * CATEASchAppRoute2
    * CATEASchAppRouteAlternateGraphic
    * CATISchApp2DZoneFrom3DZone
    * CATISchAppAssembly
    * CATISchAppAssemblyMember
    * CATISchAppCntrShow
    * CATISchAppDisplayMode
    * CATISchAppReplace
    * CATISchAppRoute2
    * CATISchAppRouteAlternateGraphic
    * CATISchCompAssemMbrLayout
    * CATISchDropOffView
    * CATISchGRRRouteAlternate
    * CATISchGRRRouteEllipse
    * CATISchReplace
    * CATISchRouteAlternateGraphic
    * CATISchematicExtension
    * CATSchDisplayMode
    * CATSchExtensionType
    * CATSchRouteAlternateGraphicStyle
  * CATTPSInterfaces framework
    * CATITPSAxisStraightness
    * CATITPSCfgAddin
    * CATITPSFactoryTTRS
    * CATITPSNonSemanticDimension
    * CATITPSView
    * CATITPSViewList
    * CATITPSWeld
  * DMAPSInterfaces framework
    * CATSPPFirstPPItemFilter
  * DNBInspectInterfaces framework
    * DNBIInsBasicServices
    * DNBIInsConCircleFtr
    * DNBIInsConCylinderFtr
    * DNBIInsConLineFtr
    * DNBIInsConPlaneFtr
    * DNBIInsConPointFtr
    * DNBIInsConSphereFtr
    * DNBIInsHitGroup
    * DNBIInsMCS
    * DNBIInsRMeas
  * Dialog framework
    * CATDlgDiaFINISHNotification
    * CATDlgDiaNEXTNotification
    * CATDlgDiaPREVIOUSNotification
    * CATDlgEditModifyExtNotification
  * DraftingInterfaces framework
    * CATIDftFramingDefinition
  * ENOVInterfaces framework
    * ENOVIBOActionManager
    * ENOVIBOEcoManager
    * ENOVICWAffectedObjects
    * ENOVICWAttachments
    * ENOVICWConfiguredViews
    * ENOVICWLinks
    * ENOVICWState
    * ENOVICatalogEvents
    * GetBOActManager
    * GetBOManager
  * ENOVaultClientCPP framework
    * VAULT_E_CORBA_SYS_EXCEPT
    * VAULT_E_CORBA_UNKNOWN_EXCEPT
    * VAULT_E_LASTCALLTOREPETE
    * VAULT_E_NOMASTER_REF
    * VAULT_E_OFFLINE
    * VAULT_E_SERVER_COMM_FAILURE
    * VAULT_E_UNKNOWN_METHOD
  * ElecHarnessItf framework
    * CATIEhiBnlSegmentExtremity
    * CATIEhiFactory
    * CATIEhiPositionPoint
  * ElecRoutingItf framework
    * CATIEwrUipBundleSegment
  * ElecSchematicItf framework
    * CATIEdiAssemblyConnector
    * CATIEdiMatingConnector
  * GSMInterfaces framework
    * CATGSMConnectType
    * CATGSMCornerType
    * CATGSMExtrapolPropagationMode
    * CATGSMLinearDraftSweepComp
    * CATGSMLinearSweepLengthType
    * CATIGSMGridSet
  * GSOInterfaces framework
    * CATIGSOFactoryGeo
  * InteractiveInterfaces framework
    * CATIIcon
    * CATIOrientationFlag
    * CATIReplacable
    * CATIReplaceUI
  * KnowledgeInterfaces framework
    * CATIInstance
    * CATIInstanceOnly
    * CATIKnowledgeAdvisorAddin
    * CATIType
    * CATIValue
  * KnowledgeInterfaces framework (created in V5R9 in LiteralFeatures. This framework was moved to KnowledgeInterfaces in V5R11)
    * CATICkeTextSheet
  * ManufacturingInterfaces framework
    * CATIMfgActivityDefaultToolAxis
    * CATIMfgDesignHoleLimits
    * CATIMfgDesignHoleParameters
    * CATIMfgFeedAndSpeedTabPage
    * CATIMfgGeometryTabPage
    * CATIMfgManufacturingUtilities
    * CATIMfgReplayToolPathCustom
    * CATIMfgTPModification
    * CATIMfgTPMultipleMotionWithProfileData
    * CATIMfgTPPackable
    * CATIMfgTPTransformation
    * CATIMfgTPUnloadData
  * Mathematics framework
    * CATCGMVirtual
  * NewTopologicalObjects framework
    * CATTopJunctionEdgeStatus
    * CATTopLyingOnDiagnostic
  * ObjectModelerBase framework
    * GetPtrSession
  * ObjectModelerSDM framework
    * CreateModel
    * CreateModelFromSchema
    * DeleteModel
    * GetContainerFromModel
    * GetModelFromContainer
    * GetModelFromFile
    * SaveFileFromModel
  * ObjectSpecsModeler framework
    * CATIBuildProvider
    * CATMakeExtensionLocal
    * CATOsmSUFactory
  * PSNInteroperability framework
    * CATCommitVPM
    * CATComputeDocId
    * CATValuatePdmId
    * GetObjectToSaveSortedList
    * OpenAPSNGraph
  * SDMRuntime framework
    * SDAILIST_HEADER
    * SDAIORDERED_HEADER
    * SDAISET_HEADER
    * SDAIUNORDERED_HEADER
    * SdaiAggrInstanceH
    * SdaiExplicitAttrH
    * SdaiInteger
    * SdaiIterInstanceH
    * SdaiModelH
    * SdaiNamedTypeH
    * SdaiReal
    * SdaiSchema
    * SdaiSchemaH
  * SurfaceMachiningInterfaces framework
    * CATISmgFactory
    * CATISmgMachiningAreaChecks
    * CATISmgMachiningAreaForbiddenZone
    * CATISmgMachiningAreaGuidingCurves
    * CATISmgMachiningAreaParts
    * CATISmgNcGeometryManager
    * CATISmgNcGeometryParameter
    * CATISmgProgramAddin
  * System framework
    * CAIIUExitDisconnectionForbidden
    * CATIUExitAutoDisconnection
    * CATIUExitDisconnectionAuthorized
    * CATLONG64
    * CATULONG64
  * ToolPathEditorInterfaces framework
    * CATIMfg3DToolPathVisu
    * CATIMfg3DToolPathVisuData
    * CATIMfgToolPathEditorHeader
    * CATIMfgToolPathEditorUserHeader
  * TopologicalOperators framework
    * CATCloseOperator
    * CATCreateCloseOperator
    * CATTopTangentCurveOperator
  * VPMDesktopObjects framework
    * CATIAVPMVDAImport
  * VPMInterfaces framework
    * CATIAVPMGenericComponent
    * CATIAVPMProductClass
    * CATIAVPMUEVersionning
    * CATICfgHistory
    * CATICfgSpecification
    * CATIVpmSCLogin
    * CATListOfCATICfgHistory_var
    * CATListOfCATICfgInterval_var
    * CATListOfCATICfgSpecANDExpression_var
    * CATListOfENOVIEventDefinition
    * ENOVIAVaultAndDocFile
    * ENOVIChangeOwnerEvent
    * ENOVIDocumentableEvent
    * ENOVIEventDefinition
    * ENOVIModifyStructureEvent
    * ENOVIRootDesktop
    * OBJECT_DATA_EVENT_INIT
    * OBJECT_EVENT_FIRE
    * VPMIListOfAttributes
    * VPMIWflUserExit
    * VPMLoginPolicy
  * VPMPersistency framework
    * ENOVEventSubFilter
    * ENOVIPubSubManager
    * ENOVPubSubDetails
  * Visualization framework
    * CATISelectShow

[Top]

* * *

History Version: **1** [Apr 2002] | Document created
---|---
[Top]

* * *

_Copyright 2002, Dassault Systmes. All rights reserved._
