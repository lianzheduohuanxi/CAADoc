---
title: "New Java Authorized APIs in CAA V5R18 SP3"
category: "general"
module: "CAACenQuickRefs"
tags: ["CATIA"]
source_file: "Doc\online\CAACenQuickRefs\CAACenV5R18SP3NewJavaAPI.htm"
converted: "2026-05-11T17:33:47.400926"
---

CAA V5 Encyclopedia |  New Java Authorized APIs in CAA V5R18 SP3  
---|---  
  
* * *

The following are the new CAA V5R18 SP3 Java Authorized APIs, compared with CAA V5R18 at SP2 level.

  * com/dassault_systemes/enovdesktop/edk0docdir/interfaces Package 
    * ENOVIContentMgtCATlet
    * ENOVIDocModel
  * com/dassault_systemes/enovdesktopappli/vdk0parteditor/interfaces Package 
    * ENOVIPartCATlet
    * ENOVIPartModel

[Top]

* * *

History Version: **1** [Dec 2007] | Document created  
---|---  
[Top]  
  
* * *

_Copyright © 1999-2007, Dassault Systèmes. All rights reserved._  
Special Notices [CAA V5 CATIA](../CAADocQuickRefs/CAADocSpecialNoticesCATIA.htm) | [CAA V5 DELMIA](../CAADocQuickRefs/CAADocSpecialNoticesDELMIA.htm) | [CAA V5 ENOVIA](../CAADocQuickRefs/CAADocSpecialNoticesENOVIA.htm)
