...
source_file: "Doc/online/CAATobUseCases/CAATobTetra.htmmd"
...
The goal of this use case is to understand the topological model of the CATIA geometric modeler, by creating a tetrahedron only with the TopologicalObjects resources. Topological objects can also be directly created by topological operators, and the use of topological operators is the recommended way rather than using the basic tools of the TopologicalObjects framework. The navigation in a topological structure is also discussed.
  ...
  * **The CAATobTetra Use Case**
    ...
    * Where to Find theCATobTetra Code
  * **Step-by-Step**
  * **In Short**
  * **References**
...
The topology manages the Boundary Representation of an object: what it bounds and how.

A _cell_ is the lowest level of topological objects. There are four types of cells:
...
Windows | `InstallRootDirectory/CAATopologicalObjects.edu/CAATobTetra.m/`

The CAATobTetra use case is made of a main named CAATobTetra.cpp located in the CAATobTetra.m module of the CAATopologicalObjects.edu framework:
Windows | `InstallRootDirectory/CAATopologicalObjects.edu/CAATobTetra.m/`
Unix | `InstallRootDirectory/CAATopologicalObjects.edu/CAATobTetra.m/`
...
To create planes, directly use the `CreatePlane` method of the `CATGeoFactory`. If NULL pointers are returned, close the factory and return an error.
...
The following code creates the `CATPLine`.
      _//PLines on Planexy_
      CATSurParam Pxy0,Pxy1,Pxy2;
      _//Gets the curve parameter corresponding to the Mathematical point
      ...
      // we might use a projection operator._
CATSurParam Pxy0,Pxy1,Pxy2;
_//Gets the curve parameter corresponding to the Mathematical point
      ...
      _// Creates the plines from the parameters_
...
```
      ...
      _// Appends the first curve in of the sim curve_
      curves.Append(piPLinexy01);
      piPLinexy01->GetLimits(crvLim1);
      _// Defines its limits_
      limits.Append(&crvLim1);
      _// Defines its relative orientation in the edge curve_
      orients.Append(**1**);   _// 1 == same orientation_

      _// The same for the second pline_
      curves.Append(piPLinexz01);
      piPLinexy01->GetLimits(crvLim2);
      limits.Append(&crvLim2);
      orients.Append(1);  _// 1 == same orientation_

      _// Now creates the sim curve_
      CATSimCurve * piSimCurve01= piGeomFactory-> **CreateSimCurve**(curves,
                                                                 limits,
                                                                 orients,
                                                                 resolution);

      _// The second sim curve_
...
Appending the points on the edge curves in the corresponding macro points, and the geometry is completed.
      _// -----------Creates the MacroPoints, representing several points
...
_// -----------Creates the MacroPoints, representing several points
      ...
      _// Adds 3 points in edge curve per macro point, because there are 3 incident edges
...
Before creating any topological entity, you first must create the factory of these entity: this factory is a CATBody, and not the CATGeoFactory. The factory of the CATBody, however, still is the CATGeoFactory.
      _// ----------- Creates the factory of the cells
...
```
      ...
      _// Associates with their geometry_
...
```
  ...
  * Bound the edge by its end vertex (`AddBoundingCell`)
    ...
    * The last argument states which poec of the macro point is the end of the edge curve
      ...
      _// Sets the geometry of Edge01: the sim curve, and the relative orientation
      // between the sim curve and the edge_
_// Sets the geometry of Edge01: the sim curve, and the relative orientation
      piEdge01->SetCurve(piSimCurve01,CATOrientationPositive);
      _// Bounds Edge01 by Vertex0,
...
```
      ...
      _// The loops define external boundary of the faces (CATLocationOuter)_
...
_// The first bounding edge Edge20
      ...
      _// Declares that the loop is finished_
      piLoopxy->Done(#);

      _// Faceyz_
      piFaceyz->AddDomain(piLoopyz);
      _// Associates with the geometry and bounds the face_
      ...
      piLoopxz->Done(#);_// The loop is finished!_
      ...
      piLoopc->Done(#);_// The loop is finished!_
...
```
      ...
      _// Bounds by the faces and add them in the shell at the same time
```

      // To define a volume, the shell orientation points to the volume inside_
_// Adds the shell_
piVolume->**AddDomain**(piShell);
_// Bounds by the faces and add them in the shell at the same time
...
```
      ...
      _// It is a shell, because it is made of faces (dimension 2)_.
...
```
        ...
        _// Is the geometry of Plane type?_
        if (NULL == (piGeom->IsATypeOf(**CATPlaneType**)) ) _// the geometry is a plane!..._
```
...
```
        ...
        _// Another way to retrieve the cells: use aboundary iterator_
...
nbEdges = nbEdges+1;
        ...
        _// There must be three edges for each face ..._
```vbscript
        if (3!=nbEdges)
